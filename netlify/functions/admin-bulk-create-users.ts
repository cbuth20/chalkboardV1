...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (authError || !user) {
      return { statusCode: 401, body: JSON.stringify({ success: false, error: 'Invalid authentication token' }) };
    }
    ...
    if (requesterError || !requester) {
      return { statusCode: 404, body: JSON.stringify({ success: false, error: 'User profile not found' }) };
    }

    if (requester.role !== 'admin') {
      return { statusCode: 403, body: JSON.stringify({ success: false, error: 'Unauthorized: Admin role required' }) };
    }
    ...
    if (membershipError || !membership) {
      return { statusCode: 404, body: JSON.stringify({ success: false, error: 'Organization membership not found' }) };
    }
    ...
    if (cleanedEmails.length === 0) {
      return { statusCode: 400, body: JSON.stringify({ success: false, error: 'At least one email is required' }) };
    }

    const invalid = cleanedEmails.filter((e) => !isValidEmail(e));
    if (invalid.length > 0) {
      return { statusCode: 400, body: JSON.stringify({ success: false, error: `Invalid email(s): ${invalid.join(', ')}` }) };
    }
    ...
    if (!resendApiKey || !emailFrom) {
      return { statusCode: 500, body: JSON.stringify({ success: false, error: 'Missing RESEND_API_KEY or EMAIL_FROM environment variables' }) };
    }
