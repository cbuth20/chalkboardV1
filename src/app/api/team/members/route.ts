  ...
  try {
    ...
    if (!membership?.org_id) {
      return NextResponse.json({
        success: true,
        data: { teamId: null, orgId: null, members: [] },
      });
    }
    ...
    if (!teamId) {
      return NextResponse.json({
        success: true,
        data: { teamId: null, orgId: membership.org_id, members: [] },
      });
    }

    // Fetch org members + basic profile fields
    ...
    // Fetch multi-positions from team_members (if present) for this team
    ...
    if (teamMembersError) {
      // Not fatal; we can still show position_code
    ...
    const members = (orgMembers || []).map((m: any) => {
      ...
      const primary = primaryByUserId.get(m.user_id) || (m.position_code ? (String(m.position_code).toUpperCase() as SkillPosition) : null);
